"""
LSTM Mejorado para IDS - Versión 2
Optimizado para maximizar recall en detección de ataques
Incluye: Bidirectional LSTM, Attention Mechanism, regularización L2
"""
...
class AttentionLayer(layers.Layer):
    """
    Attention mechanism simple para LSTM
    Permite al modelo enfocarse en pasos temporales importantes
    """
...
class IDSModelLSTM_v2:
    ...
    def load_and_preprocess(self, train_path, test_path):
        ...
        # Reshape para LSTM (batch_size, time_steps, features)
    ...
    def build_model(self, input_shape):
        ...
        - 3 capas Bidirectional LSTM con unidades progresivas
        - Attention mechanism para enfoque en features importantes
        - Regularización L2
        - Dropout adaptativo
        """
        model = keras.Sequential([
            # Primera capa Bidirectional LSTM con más unidades
            ...
            # Tercera capa LSTM con return_sequences para attention
            layers.Bidirectional(
        ...
        model.compile(
            ...
            loss=FocalLoss(alpha=0.4, gamma=2.0),
            metrics=[
                'accuracy',
    ...
    def train(self, X_train, y_train, X_val, y_val, epochs=50, batch_size=128):
        ...
        callbacks = [
            ...
            ModelCheckpoint(
                self.model_path if self.model_path else 'best_lstm_v2_model.h5',
                monitor='val_recall',
                mode='max',
                save_best_only=True,
                verbose=1
    ...
    def evaluate(self, X_test, y_test, threshold=0.35):
        """
        Evalúa el modelo con threshold optimizado
        """
    ...
    def save_scaler(self, path='scaler_lstm_v2.pkl'):
        """Guarda el scaler"""
        joblib.dump(self.scaler, path)
        print(f"Scaler guardado en: {path}")
    
    def load_model(self, model_path, scaler_path='scaler_lstm_v2.pkl'):
...
if __name__ == "__main__":
    ...
    ids_model = IDSModelLSTM_v2(model_path='best_lstm_v2_model.h5')
    ...
    ids_model.save_scaler('scaler_lstm_v2.pkl')
